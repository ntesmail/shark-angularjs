...
gulp.task('webpack-server-before', function(cb) {
    ...
    if (modules) {
        ...
        content = fs.readFileSync(path.join(config.webapp, config.jsPath, 'shark-angular.ui.js'), { encoding: 'utf8' });
    }
    console.log(content);
...
gulp.task('copy-dist-js', function() {
    return gulp.src(path.join(tmp, config.jsPath, 'shark-angular.ui.js')).pipe(gulp.dest('dist'));
});
...
gulp.task('serve-express', function(cb) {
    ...
    // index.html
    app.get('/shark-angular/index.html', function(req, res) {
    ...
    var router = sharkAutomation.registerServerRouter({
        ...
        webpack: {
            replaceEntry: {
                'js/shark-angular.ui': [path.join(__dirname, '/src/main/webapp/js/shark-angular.ui.js')]
            },
            devtool : 'source-map'
        }
    });
