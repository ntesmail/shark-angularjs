    ...
    .directive(COMPONENTS.selecter, ['sharkconfig', function(sharkconfig) {
        ...
        return {
            ...
            link: function($scope, element, attrs, ngModelCtrl) {
                ...
                // 对应的真值字段
                var actualKey = (typeof attrs.actualKey !== 'undefined' ? attrs.actualKey : SelecterConfig.actualKey);
                // 对应的展示值字段
                var displayKey = (typeof attrs.displayKey !== 'undefined' ? attrs.displayKey : SelecterConfig.displayKey);
                // 选中之后的样式
                var activeStyle = (typeof attrs.activeStyle !== 'undefined' ? attrs.activeStyle : SelecterConfig.activeStyle);
                // 如果定义了name属性，把selecter组件赋给$scope
                var selecterName = attrs.name;

                // 下拉框数据变化后，重置下拉框
                $scope.$watch(function() {
                    return $scope.$eval(attrs.selecterData);
